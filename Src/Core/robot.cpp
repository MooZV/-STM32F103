...
void publishCarData(){
    if(HAL_GetTick() - lastPublishTime < 1 ){
        return;
    }
    ...
    struct TXProtocol1* protocol = new TXProtocol1;
    uint8_t protocolLength = sizeof(TXProtocol1);
    ...
    // 侧倾角
    short beta = (short)(car.servo->getRadian()*1000);
    protocol->beta = beta;
    // 温度
    ...
    // 将底盘信息发送出去
    common_uart1_send((uint8_t*)protocol,protocolLength);
...
//    HAL_GPIO_TogglePin(GPIOE,GPIO_PIN_10);
    ...
    struct TXProtocol2* protocol = new TXProtocol2;
    uint8_t protocolLength = sizeof(TXProtocol2);
    protocol->head0 = FLAG_HEAD0;
    protocol->head1 = FLAG_HEAD1;
    protocol->type = TYPE_IMU;
    protocol->len = protocolLength;
    ...
    // 将下位机信息发送出去
    common_uart3_send((uint8_t*)protocol,protocolLength);
